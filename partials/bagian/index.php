...
<div class="shadow-xl flex flex-col gap-4 bg-white rounded-2xl p-8 h-full relative overflow-hidden">
    <form method="POST" class="flex justify-between">
        ...
        <a href="?page=save_bagian" class="bg-blue-500 py-2 px-4 rounded-lg text-white font-medium flex gap-2 items-center">
            <i>
    ...
    <table class="w-full rounded-t-lg overflow-hidden text-sm">
        <thead class="bg-blue-500 text-white">
            <tr>
                ...
                <th class="py-2 px-4 text-start">Nama Bagian</th>
        ...
        <?php 
            if(isset($cari)){
                $jumlah = "SELECT * FROM tb_bagian WHERE id_bagian LIKE '%$cari%' OR nama_bagian LIKE '%$cari%'";
            } else {
                $jumlah = "SELECT * FROM tb_bagian";
            }
            ...
            if(isset($cari)){
                $sql = "SELECT * FROM tb_bagian WHERE id_bagian LIKE '%$cari%' OR nama_bagian LIKE '%$cari%' ORDER BY id_bagian DESC LIMIT $start, $limit";
            }
            ...
            while ($data = mysqli_fetch_array($query)) {
                ...
                echo "<td class='px-4 py-2'>".$data['id_bagian']."</td>";
                echo "<td class='px-4 py-2'>".$data['nama_bagian']."</td>";
                echo "<td class='px-4 py-2 flex items-center gap-1'>";
                echo "<a href='?page=edit_bagian&id=$data[id_bagian]' class='px-2 py-1.5 bg-green-600 rounded-md text-white font-semibold'>";
...
';
                echo "</a>";
                echo "<a href='?page=data_bagian&delete=2&id=$data[id_bagian]' class='px-2 py-1.5 bg-red-500 rounded-md text-white font-semibold'>";
                echo '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">
    ...
    <div class="flex items-center mt-auto justify-between">
        <div class="flex items-center gap-2">
            <?php 
                if ($active_page > 1) {
                    echo '
                    <a href="?page=data_bagian&hal= '.($page-1).'">
